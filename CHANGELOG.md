...
### 重大变更
- 🌐 **Web 界面**: 基于 Gradio 的图形化 Web 界面
  - 友好的用户界面，无需命令行操作
  - 支持所有核心功能：订阅管理、即时检查、自定义报告
  - 实时查看仓库信息和统计数据

### 架构优化
- 🔧 **AI 模块重构**: 解耦 AI 相关代码
  - 新增 `ai_client.py`: 统一的 AI 客户端封装
  - 新增 `prompts.py`: 集中管理所有 Prompt 模板
  - 提高代码可维护性和可扩展性

### 新增
- 🎨 **Gradio Web UI** (`src/web/gradio_ui.py`)
  - 订阅管理页面：添加/移除/查看订阅
  - 即时检查页面：快速查看任意仓库更新
  - 自定义报告页面：生成指定日期范围的报告
  - 仓库信息页面：查看仓库详细统计

- 🤖 **AI 客户端封装** (`src/ai/ai_client.py`)
  - 支持 OpenAI、Anthropic、DeepSeek
  - 统一的 API 接口
  - 更好的错误处理和日志

- 📝 **Prompt 模板管理** (`src/ai/prompts.py`)
  - 系统角色提示模板
  - 更新报告提示模板
  - 每日报告提示模板
  - 便于优化和 A/B 测试
...
- 🚀 **快速启动脚本**
  ...
  - `python -m src.main web`: 命令行启动 Web 服务

### 改进
- 📦 **更新依赖**: 添加 `gradio>=4.0.0`
- � **代码组织**: AI 相关代码模块化，职责更清晰
- � **易用性**: 提供图形化界面，降低使用门槛
...
# 自定义端口和主机
python -m src.main web --port 8080 --host 127.0.0.1
